...
app.post("/api/patients/:id/drive-sync", ensureAdmin, async (req, res) => {
  try {
    ...
    if (req.user.accessToken) {
      ...
      const fileMetadata = {
        name: `Meridian-Patient-${(record.fullName || record.id).replace(/\s+/g, "_")}-${record.id}.json`,
...
async function startServer() {
  await db.initDb();
  app.listen(PORT, () => {
    console.log(`✓ Meridian Weight Clinic (Express.js) running at http://localhost:${PORT}`);
  });
}
